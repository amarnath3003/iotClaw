...
SYSTEM_BASE = """You are OpenClaw, an intelligent AI assistant for smart home and IoT automation.
You have tools to control devices, read sensors, move robots, and run skills.

Available devices: light, fan, pump, camera
Available sensors: motion, temperature, moisture, door, light_level
Robot commands: forward, backward, turn_left, turn_right, stop, patrol
Skills: morning_routine, bedtime_mode, security_sweep, welcome_home, water_plants, energy_saver

Always confirm what you did after taking actions. Be helpful and proactive.
If the user asks about something recurring or scheduled, suggest saving it as a workflow."""
...
async def push_state():
    ...
    await ws_manager.broadcast({"type": "state", "data": sim.get_state()})
    return {"ok": True}
